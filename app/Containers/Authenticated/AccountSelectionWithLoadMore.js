...
/* Exporting class */
class AccountSelectionWithLoadMore extends Component {
        ...
        if (this.state.searchString.trim() !== '') {
            if (this.state.searchString.trim().length < 3) {
                const arrSearchResult = this.state.dataSourceMain.filter((obj) => {
                    return obj.value_title.toLowerCase().startsWith(this.state.searchString.toLowerCase()) //obj.startsWith(value)
                })
    ...
    fetchAccount(isSearch = false) {
        ...
        https://api.sherpadesk.com/accounts?is_with_statistics=false&limit=25&search=bar*
        if (isSearch) {
        ...
        ApiHelper.getWithParam(ApiHelper.Apis.Accounts, objData, this, this.state.page == 0, authHeader).then((response) => {
            if (this.state.page !== 0 && response) {
                ...
                if (response) {
                    var arrAccounts = []
                    if (!isSearch && this.props.defaultOption) {
...
export default connect(mapStateToProps)(AccountSelectionWithLoadMore);