...
// Styless
import styles from './Styles/TechnicianTicketsStyles'
  ...
  componentDidMount() {
    ...
    CommonFunctions.retrieveData(UserDataKeys.Config)
      .then((response) => {
        ...
        this.setState({ tickets: tickets, ticketTitle: ticketTitle, config: config })
        // Call Apis
      })
      ...
      <Container>
        ...
        <SafeAreaView>
          <NavigationBar
            ...
            showTitle={this.state.ticket ? `${this.state.tickets} for ${this.state.ticket.firstname + ' ' + this.state.ticket.lastname}` : 'Tickets'}
          />
        </SafeAreaView>
...
export default connect(mapStateToProps)(TechnicianTickets);