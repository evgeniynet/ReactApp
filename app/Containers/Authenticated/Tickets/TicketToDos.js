...
// Styless
import styles from './Styles/TicketToDosStyles'

class TicketToDos extends Component {
  ...
  fetchToDos = async () => {
    ...
    let objData = { c:1, ticket: this.props.ticket.id, is_completed: false, all_item_types: true, is_sub_view: false }
    ApiHelper.getWithParam(ApiHelper.Apis.ToDos, objData, this, true, authHeader).then((response) => {
      var arrMainTodos = []
     

      response.forEach(elementMain => {
        var elementMain = { ...elementMain }
        elementMain.sub = [{ ...elementMain }]
        ...
        if (arrSubTodos.length > 0) {
          var isAdded = false
          for (let index = 0; index < arrMainTodos.length; index++) {
            const elementTempMain = arrMainTodos[index];
            if (elementTempMain.list_id == elementMainObj.list_id) {
              if (elementTempMain.sub && elementTempMain.sub.length > 0) {
                isAdded = true
                arrMainTodos[index].sub.push(elementMainObj.sub[0])
              } else {
                isAdded = true
                arrMainTodos[index].sub = elementMainObj.sub
              }
            }
          }
          if (!isAdded) {
...
export default connect(mapStateToProps)(TicketToDos);