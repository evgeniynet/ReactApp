...
// Styless
import styles from './Styles/QueuesStyles'
  ...
  searchNow() {
    if (this.state.searchString.trim() !== '') {
      const arrSearchResult = this.state.dataSourceMain.filter((obj) => {
        ...
        return obj.fullname.toLocaleLowerCase().includes(strSearch) //obj.startsWith(value)
      })
    ...
    return (
      <Animatable.View useNativeDriver={true} animation={this.state.searchString.trim().length == 0 ? 'fadeInUpBig' : 'pulse'}>
        <CardItem
          ...
          <Label style={styles.titleText}>{row.item.fullname}</Label>
